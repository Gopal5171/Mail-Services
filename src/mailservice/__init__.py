...
from mailservice.util.log import init_log
...
class MailService:
    ...
    def __retrieve_updates(self):
        self.check_expired_token()
        mails = self.get_updates()
        self.__process_new_updates(mails)
        self.__process_new_messages()

    def check_expired_token(self):
    ...
    def __process_new_updates(self, mails: list[Mail]):
        """
        ...
        for mail in self.list_mail:
            ...
            for message_handler in self.mail_handlers:
                if self.regex_mail_subject(mail.subject, message_handler["subject_regex_pattern"]):
                    ...
                    self.__exec_task(message_handler["function"], mail)
                    break
    ...
    def completed_mail(self, mail: Mail, is_success: bool):
        ...
        """
        if is_success:
            mark_seen_and_move(self.config["USER_NAME"], self.access_token, int(mail.id), "done")
            self.logger.info(
                "mail id: "
                + str(mail.id)
                + ", subject: "
            ...
            mark_seen_and_move(self.config["USER_NAME"], self.access_token, int(mail.id), "failed")
            self.logger.info(
                "mail id: "
                + str(mail.id)
                + ", subject: "
